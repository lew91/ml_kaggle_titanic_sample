...
# %matplotlib inline
g = sns.heatmap(
    dftrain_raw[["Survived", "Pclass", "Age", "SibSp", "Parch", "Fare"]].corr(),
    annot=True,
    fmt=".2f",
    cmap="coolwarm",
)
...
g = sns.catplot(
    x="SibSp", y="Survived", data=dftrain_raw, kind="bar", height=6, palette="muted"
)
g.despine(left=True)
...
g = sns.catplot(
    x="Parch", y="Survived", data=dftrain_raw, kind="bar", height=6, palette="muted"
)
g.despine(left=True)
g = g.set_ylabels("survival probability")
...
g = sns.kdeplot(
    dftrain_raw["Age"][(dftrain_raw["Survived"] == 0) & (dftrain_raw["Age"].notnull())],
    color="Red",
    shade=True,
)
g = sns.kdeplot(
    dftrain_raw["Age"][(dftrain_raw["Survived"] == 1) & (dftrain_raw["Age"].notnull())],
    ax=g,
    color="Blue",
    shade=True,
)
...
g = sns.catplot(
    x="Pclass", y="Survived", data=dftrain_raw, kind="bar", height=6, palette="muted"
)
g.despine(left=True)
g = g.set_ylabels("survival probability")
...
g = sns.catplot(
    x="Embarked", y="Survived", data=dftrain_raw, height=6, kind="bar", palette="muted"
)
g.despine(left=True)
g = g.set_ylabels("survival probability")
# Explore Pclass vs Embarked
g = sns.catplot(
    "Pclass", col="Embarked", data=dftrain_raw, height=6, kind="count", palette="muted"
)
g.despine(left=True)
...
for classifier in classifiers:
    cv_results.append(
        cross_val_score(
            classifier, X_train, y=Y_train, scoring="accuracy", cv=kfold, n_jobs=8
        )
    )
...
gsadaDTC = GridSearchCV(
    adaDTC, param_grid=ada_param_grid, cv=kfold, scoring="accuracy", n_jobs=8, verbose=1
)
...
gsExtC = GridSearchCV(
    ExtC, param_grid=ex_param_grid, cv=kfold, scoring="accuracy", n_jobs=8, verbose=1
)
...
gsRFC = GridSearchCV(
    RFC, param_grid=rf_param_grid, cv=kfold, scoring="accuracy", n_jobs=8, verbose=1
)
...
gsGBC = GridSearchCV(
    GBC, param_grid=gb_param_grid, cv=kfold, scoring="accuracy", n_jobs=8, verbose=1
)
...
gsSVMC = GridSearchCV(
    SVMC, param_grid=svc_param_grid, cv=kfold, scoring="accuracy", n_jobs=8, verbose=1
)
